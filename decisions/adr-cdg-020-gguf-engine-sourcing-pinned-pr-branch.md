...
**Status**: proposed (2026-08-04) — **ratification and the pin choice are gated on the #131 rung-1 inference/telemetry probe readback** (un-run, GPU-window-gated). This document is structured so rung-1's results *resolve* its named open questions rather than invalidate its decisions: the sourcing posture (§Decision 1), packaging shape (§Decision 3), supersession (§Decision 4), and the protocol-boundary invariant (§Decision 5) stand independent of rung-1; only the pin selection (§Decision 2, #24423 vs #24427) and two falsifiable H0s wait on it.
**Date**: 2026-08-04
...
- [ ] **(A) Pin selection: #24423 vs #24427.** **Resolution:** decided by the #131 rung-1 readback against the §Decision-2 criteria (frame-contract reachability decisive). Until rung-1 returns, `c3fb972` (#24423) is the rung-0-de-risked build default, not the ratified pin. **This ADR does not ratify until this resolves.**
- [ ] **(B) H0: the visibility delta (Tier-0 `DiffusionFrame` emission) is implementable in the example/serving layer without kernel work.** **Resolution:** falsified-or-confirmed by rung-1 item 2 (#24423 visual-server frame capture vs Tier-0 contract) and item 1 (visibility-tax measurement). If falsified (kernel work required), Decision 3's "maintenance = adapter + SHA" scope widens and the ADR returns to the operator before acceptance.
- [ ] **(C) H0: an unoptimized, visible engine reaches usable tok/s on 3090-class hardware.** **Resolution:** rung-1 item 5 (Q4_K_M tok/s on sm_75 as the Turing datapoint; 3090/sm_86 via the #16 cross-machine lane). Bears on whether the shipped envelope is a real product or a demo.
- [ ] **(D) H0: #24427's top-k self-cond truncation does not materially alter convergence.** **Resolution:** rung-1 item 3 (three-path conformance run). Either answer is upstream-grade evidence; a material difference reweights §Decision-2 criterion 3.
