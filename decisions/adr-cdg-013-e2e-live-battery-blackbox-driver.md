...
**Status**: accepted (ratified by independent Opus design-gate review, 2026-07-13; operator veto standing)
**Date**: 2026-07-13
...
2. **Headless server lifecycle, per battery run.** A session-scoped fixture launches ComfyUI headless (`python main.py --listen 127.0.0.1 --port 8199 --output-directory <tmp> --disable-auto-launch`, `<comfyui-root>/.venv` interpreter, port isolated from the operator's interactive 8188), polls `/object_info` for readiness, yields base URL + websocket `client_id`, and on teardown `SIGTERM`s and reaps.
...
- **Composes with CDG-012, doesn't duplicate it.** CDG-012 DV.2 is static (load `examples/*.json`, validate against node defs, no server); this battery is the live tier that POSTs the same graphs to the real model. Static proves *matches the node defs*; live proves *produces honest output*. CDG-012's KV_CACHE workflows drop into the battery's S9 slot and feed the same merged-coverage dataset. **The coverage seam composes cleanly over that one dataset (design-gate finding, 2026-07-13):** this ADR deliberately sets *no* new repo-wide `fail_under` (which would collide with the `dgemma/model.py` gap CDG-012 DV.1 explicitly protects), and CDG-012's DV.1 floor is enforced *not* by `fail_under` but by a dedicated allowlist gate test (issue #62 §H, option b) that reads the combined `.coverage` and asserts *named* channel modules at 100% — naming model.py nowhere, so it cannot regress that gap. The two mechanisms read the same `coverage combine` dataset #59-§3 produces and do not compete: this battery's contribution is *confirmation the live path reaches the fake-planted boundary rows*, DV.1's is a *positive per-module floor on the KV_CACHE modules* — orthogonal assertions over one merged dataset.
...
**Named precondition defect (not a step this ADR takes):** the ComfyUI custom_nodes symlink `<comfyui-root>/custom_nodes/ComfyUI-DiffusionGemma` currently points at `<dev-root>/cdg-feat21-review`, which no longer exists — a dead symlink that blocks the pack from loading. Repointing it at the canonical repo path touches the shared ComfyUI install and is operator-gated (issue #59 §5).
