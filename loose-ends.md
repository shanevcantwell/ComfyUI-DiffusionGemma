...
## 2026-07-05 — Scheduler-variant candidates from external sampler brainstorm (filtered): temperature field, cluster-commit, branch/prune

**Category:** banked idea (unbuilt) — candidates for the P4 scheduler/curve zoo
and for #7's morphology comparison set
**Related ADR:** ADR-CDG-004 (swappable-scheduler seam — the joint all three
plug into); issues #7 (commit-front morphology), #6 (adversarial renoise)
**Graduation trigger:** any variant that ships as a real scheduler gets an ADR
(commit semantics and coupling metrics are decision surface once built on).

### Context
Operator ran an external brainstorm (Gemini) on alternative samplers. Most of
it re-derived the entropy-bound commit rule in mask-based vocabulary
(rejected: DG has no mask — ADR-CDG-001; its "freeze/lock" is the scheduler's
commit state, which `EntropyBoundScheduler` already implements) and
independently converged on #7's crystallization framing — convergent
validation of the banked experiment, not new work. Three deltas survived the
filter.

### Decision (what's banked)
1. **Per-position temperature field** — cool logits adjacent to committed
   spans, keep distant regions hot. Pure scheduler variant, needs no joint
   distribution. The most implementable delta.
2. **Cluster-commit** — commit multi-token spans jointly on inter-position
   *coupling* rather than per-position entropy. **Named catch (load-bearing):**
   one forward pass emits per-position *marginals*; a product of marginals
   carries zero inter-position covariance, so PMI/MI between positions is not
   computable from a single pass's logits (the brainstorm's `torch.roll`
   sketch computes nothing of the sort). Real routes: multi-sample empirical
   co-occurrence (costly); counterfactual probe (commit *i*, measure Δentropy
   at *j* next pass — #7's commit-event-over-steps data yields this signal
   for free); attention weights as a cheap coupling proxy.
3. **Branch/prune best-of-N** — clone the canvas into N batch streams, prune
   on canvas-level entropy, re-clone survivors. Known shape in the literature
   (particle-filter / Feynman–Kac steering for diffusion); VRAM cost is the
   batch dim; fits DG's parallel canvas natively. Also the natural mechanism
   for the elastic-gap-width entry below.

### Why Not an ADR?
- [ ] Hard to reverse? → Nothing built; all three are swappable scheduler
      variants behind the ADR-CDG-004 seam.
- [x] Surprising without context? → The marginals catch is, hence this entry.
- [ ] Real trade-off? → Not yet load-bearing; becomes one at build time.

---

## 2026-07-05 — Elastic gap width for the inpaint workflow (#3 delta): sweep-and-select, because "empty" has no native token

**Category:** banked idea (unbuilt) — extends issue #3's fixed-width infill
semantics
**Related ADR:** ADR-CDG-001 (no absorbing state); issue #3 (gap-fill
necessity probe); DGemmaRenoise entry above (mask mechanics)
**Graduation trigger:** rides `DGemmaInpaintMask` — an ADR if elastic width
becomes UI surface with its own selection semantics.

### Context
#3 names fixed-width infill as a hard semantic (a 5-token mask yields exactly
5 tokens) with *manual* width sweep as mitigation. Operator delta: bound the
gap by a max token length but let effectively-shorter fills win — "empty
results could shorten it."

### Decision
"Shorter" cannot be *emitted* natively — DG has no absorbing/empty state and
canvas width is fixed, so a surplus slot always holds a real token. Elastic
width must therefore be *selected*, not generated: run the width sweep as a
batch dimension (mask k=1..K across streams — the branch/prune loop in the
entry above), select by per-position entropy inside the gap and/or cross-run
agreement (#3's signal 3). **Named failure mode to price in the selection
metric:** a too-wide gap gets padded with low-content glue tokens that can
still score confidently low entropy — the metric needs a degenerate-glue
penalty, not entropy alone.

### Why Not an ADR?
- [ ] Hard to reverse? → Unbuilt; a selection loop over the existing sweep.
- [x] Surprising without context? → The "empty has no token, so shorter is
      selected not generated" point is, hence this entry.
- [ ] Real trade-off? → The selection metric is, but only once built.
