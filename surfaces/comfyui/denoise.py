...
class DGemmaDenoise:
    ...
    DESCRIPTION = (
        ...
        "(IN-2); leave it unwired for an unconditioned run. prompt is the "
        "TURN, kv_cache is independent CONTEXT — they compose (ADR-CDG-024, "
        "issue #257): a non-empty prompt alongside a connected kv_cache is "
        "chat-templated and prefilled onto the cache before decoding "
        "begins (this is what the run crystallizes onto). Leave prompt "
        "empty alongside a connected kv_cache for pure injection — this "
        "deliberately does NOT anneal to coherent text. Two rejections to "
        "recognize: (1) gen_length > canvas_length with both prompt and "
        "kv_cache wired is blocked pending #263's multi-block splice fix — "
        "shrink gen_length to one canvas_length or clear prompt instead; "
        "(2) a kv_cache already grown by a prior composed run (e.g. a "
        "cached DGemmaEncode node output reused across runs) is rejected — "
        "re-run DGemmaEncode (#265). Outputs: text "
        ...
        return {
            "required": {
                ...
                "prompt": (
                    "STRING",
                    {
                        ...
                        "default": "",
                        "tooltip": (
                            "The TURN: chat-templated (role markers + "
                            "generation-prompt suffix) and, when kv_cache "
                            "is connected, prefilled onto the cache before "
                            "decoding — this is what generation "
                            "crystallizes onto (ADR-CDG-024, issue #257). "
                            "EMPTY prompt alongside a connected kv_cache is "
                            "pure injection, which deliberately does NOT "
                            "anneal to coherent text. Rejected if gen_length "
                            "> canvas_length while both prompt and kv_cache "
                            "are set — multi-block composed runs are "
                            "blocked pending #263's splice-offset fix; "
                            "single-block composed and multi-block pure "
                            "injection remain allowed."
                        ),
                    },
                ),
                "seed": (
            ...
            "optional": {
                "kv_cache": (
                    ...
                    {
                        "tooltip": (
                            "Optional KV-cache from DGemmaEncode: "
                            "independent CONTEXT (raw-encoded, no chat "
                            "framing) that conditions this run as "
                            "background — it cannot restrict output the "
                            "way prompt does. Leave unwired for an "
                            "unconditioned run. Composes with prompt "
                            "(ADR-CDG-024, issue #257): a non-empty prompt "
                            "is the current-turn text, chat-templated and "
                            "prefilled onto this cache before decoding. "
                            "Leave prompt empty for pure injection (no "
                            "prefill — does not anneal to coherent text). "
                            "Rejected if this cache was already grown by a "
                            "prior composed run — e.g. a cached "
                            "DGemmaEncode node output reused across runs; "
                            "re-run DGemmaEncode to mint a fresh cache "
                            "(#265). Also rejected together with a "
                            "non-empty prompt when gen_length exceeds one "
                            "canvas_length — multi-block composed runs are "
                            "blocked pending #263's splice-offset fix."
                        )
                    },
                ),
